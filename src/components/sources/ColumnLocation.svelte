<script lang="ts">
  ...
  let columnTypes: { text: string; value: ColumnConfig }[] = [
    { text: 'Named', value: { Name: '' } },
    { text: 'At index', value: { Index: 0 } },
    { text: 'Named at index', value: { NameAtIndex: ['', 0] } },
  ]
  function getIndex(c: ColumnConfig) {
    if ('Name' in c) return 0
    else if ('Index' in c) return 1
    else if ('NameAtIndex' in c) return 2
    else return 0
  }
